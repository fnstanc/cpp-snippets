...
template<typename Guard, typename Function>
WeakCallback<Guard, Function> makeWeakCallback(Guard &&g, Function &&f)
{
    return WeakCallback<Guard, Function>(
        std::forward<Guard>(g), std::forward<Function>(f));
}
